...
class GoodsReceiveController extends Controller
{
    ...
    public function edit($id) {
        $goodsReceive = DB::table('transaction.t_goods_receive as goods_receive')
                        ->select('purchase.id as purchase_ids','purchase.po_no', 'supplier.name', 'goods_receive.gr_no', 'goods_receive.date', 'goods_receive.receiver')
                        ->join('transaction.t_purchase as purchase', 'purchase.id', '=', 'goods_receive.purchase_id')
                        ->join('master.m_supplier as supplier', 'supplier.id', '=', 'purchase.supplier_id')
        ...
        $materials = DB::table('transaction.t_detail_purchase as detail_purchase')
                    ->select('detail_purchase.id', 'material.name', 'detail_purchase.width','detail_purchase.quantity')
                    ...
                    ->get();
        // $detail = DB::table('transaction.t_detail_goods_receive as goods_receive')
        return view('transaction.procurement.goods-receive.edit', compact('goodsReceive', 'materials'));
    }
